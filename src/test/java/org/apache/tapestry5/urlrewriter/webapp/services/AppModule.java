...
import org.apache.tapestry5.urlrewriter.webapp.pages.URLRewriteSuccess;

@SubModule(UrlRewriterModule.class)
public class AppModule {

	private static final String SUCCESS_PAGE_NAME = URLRewriteSuccess.class
	...
	public static void contributeURLRewriter(
		...
		URLRewriterRule rule4 = new URLRewriterRule() {
			...
			public Request process(Request request, URLRewriteContext context) {
				...
				String path = request.getPath();
				if (serverName.equals(IntegrationTests.SUBDOMAIN)
						&& path.equals("/")) {
					path = String.format("/%s/%s", SUCCESS_PAGE_NAME,
							IntegrationTests.LOGIN);
					request = new SimpleRequestWrapper(request, path);
				}
				return request;
	...
	// note that as this is a test, there are a lot of shortcuts employed in the
	// url processing.
	...
	static class Rule7 implements URLRewriterRule {
		...
		private Request decodePage(Request request) {
			...
			String newPath = "/" + reverse(pageName) + pathRemainder;
		...
		private Request decodeComponentLink(Request request, String path,
				String event) {
			int idx = path.indexOf('.');
			String pageName;
			String componentName = null;// idea complains about componentName
		...
		public Request process(Request request, URLRewriteContext context) {
			if (context.isIncoming()) {
				...
				// page link reversing is just to illustrate the fact that we
				// can manipulate paths without caring about
				// the precise details of the path. Except, we don't want to
				// mess with index, URLRewriteSuccess,
				// or dummy because that messes up the rest of the tests.
				String pageName = context.getPageParameters()
						.getLogicalPageName().toLowerCase();
				if (pageName.equals("index")
						|| pageName.equals("urlrewritesuccess")
						|| pageName.equals("dummy")) {
					return request;
				}
				String newPath = "/rpage"
						+ request.getPath().replaceAll(pageName,
								reverse(pageName));
				return new SimpleRequestWrapper(request, newPath);
			} else {
				// mangle the event details.
				String pageName = context.getComponentEventParameters()
						.getActivePageName().toLowerCase();
				if (pageName.equals("index")
						|| pageName.equals("urlrewritesuccess")
						|| pageName.equals("dummy")) {
					return request;
				}
				String eventName = context.getComponentEventParameters()
						.getEventType().toLowerCase();
				String componentId = context.getComponentEventParameters()
						.getNestedComponentId().toLowerCase();

				String newPath = "/cevent"
						+ request.getPath()
								.replaceAll(pageName, reverse(pageName))
