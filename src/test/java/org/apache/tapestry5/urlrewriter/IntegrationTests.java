...
public class IntegrationTests extends SeleniumTestCase {
	...
	public void test_link_rewriting_without_virtual_host() {
		...
		final String url = String.format("http://%s:%d/", SUBDOMAIN, 9090);
		// JETTY_PORT);
	...
	public void test_url_rewriter_using_context() throws InterruptedException {
		...
		assertAttribute("//a[@class='action']/@href",
				"/cevent/txetnochtiwetirwer.knilgnitcaeht");

		clickAndWait("//a[@class='page']");
		assertTextPresent("URL Rewriting that relies on the RewriteContext evidently works.");

		// note that we should be able to get back to rewritewithcontext via
		// rpage encoding/decoding...
